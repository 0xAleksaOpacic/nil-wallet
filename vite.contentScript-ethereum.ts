...
export default defineConfig({
  build: {
    outDir: resolve(outDir, 'content_ethereum'), // Content scripts output directory
    rollupOptions: {
      input: {
        injected: resolve(root, 'contentScripts', 'ethereum.ts')
      },
      output: {
        entryFileNames: 'ethereum.js', // Generate single file for each input
      },
    },
